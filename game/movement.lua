        ...
        if (not something_moved and not has_flipped) then
          ...
          for _,unit in ipairs(moving_units) do
            ...
            if #unit.moves > 0 and not unit.removed and unit.moves[1].times > 0 then
              local data = unit.moves[1]
              if data.reason == "walk" and flippers[unit.id] ~= true and (unit.rotate or hasProperty(unit,"rotabl")) and not hasProperty(unit, "stubbn") and not hasProperty(unit,"loop") and timecheck(unit,"be","walk") then
